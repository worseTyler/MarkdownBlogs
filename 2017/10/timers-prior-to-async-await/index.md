...
The development team designed `System.Windows.Forms.Timer` specifically for use within a rich client user interface. Programmers can drag it onto a form as a non visual control and regulate the behavior from within the Properties window. Most importantly, it will always safely fire an event from a thread that can interact with the user interface.
