...
Since we can’t derive from the Xamarin.Forms Binding (sealed class) nor BindingBase (constructor is declared internal) classes, we will declare our MultiBinding class as a [IMarkupExtension<Binding>](https://docs.microsoft.com/en-us/dotnet/api/xamarin.forms.xaml.imarkupextension-1?view=xamarin-forms). It [is important](https://bugzilla.xamarin.com/show_bug.cgi?id=37622) to use IMarkupExtenion<T> rather than [IMarkupExtension](https://developer.xamarin.com/api/type/Xamarin.Forms.Xaml.IMarkupExtension/) if you are going to use the [XAML Compile](https://developer.xamarin.com/guides/xamarin-forms/user-interface/xaml-basics/xamlc/) feature.
